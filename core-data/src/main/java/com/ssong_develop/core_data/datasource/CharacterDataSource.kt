...
class CharacterDataSource @Inject constructor(
    ...
    private val dataSourceRateLimiter = RateLimiter<String>(timeout = 1, timeUnit = TimeUnit.SECONDS)

    suspend fun getCharacterEpisode(urls: List<String>) =
        urls.takeIf { dataSourceRateLimiter.shouldFetch(urls.first()) }?.map { url -> service.getEpisodes(url) }
}